...
class Storage {
  ...
  async init() {
    try {
      ...
      let dataDir = envDir
        ? path.resolve(envDir as string)
        : path.join(__dirname, '../../data');

      // If that directory is not writable/usable, fall back to cwd/data
      try {
        ...
        try {
          ...
          // If we still fail, log and let open() report the error (with full stack)
  ...
  logger.info(`Using data directory for DB: ${dataDir}`);
      ...
      logger.error(`Failed to initialize storage: ${error && error.stack ? error.stack : error}`);
      throw error;
    }
  }
